...
var myBott = ['ub2d86b0a2f11d44df5b61e5211389041','u6697e3ffeaa87db0c9ef59b3f24b6cc6'];//TAMID LU DISINI SUPAYA BISA PKE COMMAND STAFF

const myBot = ['ub2d86b0a2f11d44df5b61e5211389041','u6697e3ffeaa87db0c9ef59b3f24b6cc6'];//TARO MID LU DISINI
           ...
           if(seq.contentType == 13 && this.sendStaff == 2 && isAdminOrBot(seq.from))
{
    ...
    await this._sendMessage(seq,'Sukses Menghapus Staff~');
        ...
        if(txt == 'response name') {
           if(isAdminOrBot(seq.from) || isStaffOrBot(seq.from)) {
            this._sendMessage(seq, 'TB 1 Hadir 􀂳');
           }
        }
        ...
        if(txt == 'halo') {
          if(isAdminOrBot(seq.from) || isStaffOrBot(seq.from)) {
        this._sendMessage(seq, 'Halo Juga Admin Atau Staff Twopay Bot');
        }
      else
        {
        ...
        if(txt == 'speed') {
            ...
            await this._sendMessage(seq,'Sabar Pay..........');
...
let { listMember } = await this.searchGroup(seq.to);
     const mentions = await this.mention(listMember);
        ...
        if(txt == 'intip') {
            this._sendMessage(seq, `Mulai pengintaian...😎😎`);
        ...
        if(txt == 'setlastpoint') {
            ...
            this.checkReader = []
            this._sendMessage(seq, `Menghapus Data Tukang Thesah...😀😀😀`);
        }  
        ...
        if(txt == 'creator bot') {
           this._sendMessage(seq, 'My Creator Is 💥🕉Mr2pay🕉✒\nId Line : http://line.me/ti/p/~mr_twopay\n\n-Twopay Bot-');
           seq.contentType=13;
           seq.contentMetadata = { mid: 'u8b2bb5214c1afc77d06901a535c23e30' };
           this._client.sendMessage(1, seq);
        }
        ...
        if(cmd == 'spam' && isStaffOrBot(seq.from)) {
            for(var i= 0; i < 10;  i++) {
               this._sendMessage(seq, 'I Love You My darling');
        }
    }
        ...
        if(txt == 'bye') {
           if(isAdminOrBot(seq.from) || isStaffOrBot(seq.from)){
          let txt = await this._sendMessage(seq, 'Kami Dari TwopayBog (TB) Terima Kasih Atas Groupnya Dan Kami Izin Leave~');
